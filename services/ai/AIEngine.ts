...
export function localAIValidate(ctx: AIContext): AIValidation {
  ...
  const hardBlock =
    isExhaustion(rsiValue, direction) ||
    (trend?.signal !== 'NEUTRAL' && trend?.signal !== direction && pullback?.signal !== direction) ||
    winProb < 58 ||
    (confidence && !confidence.approved);

  const approved = !hardBlock && winProb >= 55;
...
JSON apenas: {"approve":bool,"winProbability":0-95,"confidence":0-100,"reason":"pt curto"}`;
  ...
  try {
    ...
    const approve = Boolean(parsed.approve) && winProbability >= 62;
...
export async function validateWithAI(
  ...
  if (options?.skipRemote) {
    return {
      ...local,
      approved: true,
      winProbability: Math.max(52, Math.min(92, local.winProbability)),
      reason: local.reason.replace('não atendidos', 'ajustados (INSTANT)'),
    };
  }
  return local;
}
