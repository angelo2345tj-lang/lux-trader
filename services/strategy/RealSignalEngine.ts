...
export class RealSignalEngine {
  ...
  private static async runAnalyze(
    ...
    if (
      ...
      institutional.score < 45
    ) {
      return {
        signal: null,
        blockReason: 'Manipulação detectada — sinal bloqueado',
    ...
    const minScore = timingMode === 'INSTANT' ? 48 : 62;
    const minAgreement = timingMode === 'INSTANT' ? 28 : 45;
    ...
    if (
      ...
      const fb = buildGuaranteedSignal({
        assetSymbol,
        timeframe,
        banca,
        riskPercent,
        ...
        blendedScore,
        timingMode,
        blockNote: 'Range sem BOS — sinal cauteloso',
        winProbability: 52,
      });
      return { signal: fb, score: fb.score, timingMode, dataSource: 'fallback' };
    }
    ...
    const ai = await validateWithAI(
      ...
      { skipRemote: timingMode === 'INSTANT' }
    );

    const instantOk =
      timingMode === 'INSTANT' &&
      blendedScore >= minScore &&
      ensemble.agreement >= minAgreement;
    const confirmedOk =
      timingMode !== 'INSTANT' &&
      blendedScore >= minScore &&
      (confidence.approved || blendedScore >= 75) &&
      (ai.approved || blendedScore >= 78) &&
      ensemble.agreement >= minAgreement;

    let blockNote: string | undefined =
      consolidationNote ?? microAdvisory;
    if (!instantOk && !confirmedOk) {
      blockNote = [
        blockNote,
        blendedScore < minScore
          ? `Score ${blendedScore}% (mín. ${minScore}%)`
          : `Setup ajustado — score ${blendedScore}%`,
      ]
        .filter(Boolean)
        .join(' · ');
    ...
    const baseSignal: TradeSignal = {
      ...
      strength:
        ai.winProbability >= 78 || confidence.classification === 'ELITE'
      ...
      realRisk: riskAmount,
      mainReason: `${confidence.classification} · ${ai.winProbability}% · ${timingMode}`,
    ...
    return {
      signal: { ...signal, winProbability: ai.winProbability },
      score: signal.score,
