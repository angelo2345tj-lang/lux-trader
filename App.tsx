...
const SafeLiftAlert = ({ type, onClose, t }: { type: 'GOAL' | 'STOP', onClose: () => void, t: any }) => {
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-[5000] flex items-center justify-center p-6 bg-black/95 backdrop-blur-3xl animate-in fade-in duration-1000">
      <div className={`w-full max-w-sm glass-morphism p-12 rounded-[4rem] text-center space-y-10 border-2 ${type === 'GOAL' ? 'border-green-500/40 shadow-[0_0_100px_rgba(34,197,94,0.3)]' : 'border-red-500/40 shadow-[0_0_100px_rgba(239,68,68,0.3)]'}`}>
        ...
        <button onClick={onClose} className="w-full py-7 bg-white text-black rounded-3xl font-black uppercase text-[12px] tracking-[0.4em] shadow-2xl transition-all active:scale-95 border-b-6 border-zinc-300 italic">
          RECONHECER PROTOCOLO
        </button>
      </div>
    </div>
  );
...
const [isInitializing] = useState(false);
  ...
  const [settings, setSettings] = useState<UserSettings>(() => {
    try {
      ...
      return saved ? JSON.parse(saved) : {
        notificationsEnabled: true, soundEnabled: true, hapticFeedback: true, riskPercent: 1.5, 
  ...
  const [history, setHistory] = useState<TradeHistoryItem[]>(() => {
    try {
      const saved = localStorage.getItem('lux_history');
      return saved ? JSON.parse(saved) : [];
    } catch (e) { return []; }
  });

  const { price: livePrice, status: wsStatus } = useWebSocket(selectedAsset.symbol, selectedTimeframe);
  ...
  useEffect(() => {
    setDataProvider(getProviderForSymbol(selectedAsset.symbol));
    fetchCandles(selectedAsset.symbol, selectedTimeframe, 80).catch((e) =>
      logger.error(e instanceof Error ? e.message : 'Erro candles', 'App')
    );
  ...
  useEffect(() => { localStorage.setItem('lux_history', JSON.stringify(history)); }, [history]);
  ...
  useEffect(() => {
    if (isGoalReached && !safeLiftAlert) setSafeLiftAlert('GOAL');
    if (isStopReached && !safeLiftAlert) setSafeLiftAlert('STOP');
  }, [isGoalReached, isStopReached, safeLiftAlert]);
  ...
  const handleScanner = useCallback(async () => {
    ...
    if (isGoalReached || isStopReached) {
    ...
    try {
      ...
      if (result.signal) {
        ...
        setScanError(null);
        setWinProbability(result.winProbability ?? result.signal.winProbability);
        playSFX(SFX.SIGNAL);

        if (execution.autoExecute && settings.autoTradeEnabled) {
          execution.executeSignal(result.signal, userBanca).then((r) => {
            if (r.success) setGlobalSuccess('ORDEM EXECUTADA — ' + r.message);
            else logger.warn(r.message, 'execution');
          });
        }

        if (isIdealSignal(result.signal, result.winProbability)) {
          setIdealPopupSignal(result.signal);
          setIdealWinProb(result.winProbability);
          if (settings.notificationsEnabled) {
            requestNotificationPermission().then((granted) => {
              if (granted) sendDeviceNotification(result.signal!, result.winProbability, true);
            });
          }
        ...
        setScanError(
          result.blockReason?.includes('consolidação')
            ? 'Setup em ajuste — tente modo Instantâneo ou outro TF'
            : result.blockReason ?? 'Análise sem sinal — tente novamente'
        );
      }
    } catch (e) {
  ...
  const handleSaveOperation = (item: TradeHistoryItem) => {
    ...
    setGlobalSuccess('OPERAÇÃO SALVA');
    setTimeout(() => setGlobalSuccess(null), 3000);
  };
  ...
  const saveSignalToHistory = useCallback(
    (sig: TradeSignal) => {
      ...
      handleSaveOperation({
        ...
        risks: sig.risks,
        aiExplanation: sig.aiExplanation,
        session: sig.session,
        indicatorSnapshot: sig.indicatorCalculations,
        utcLabel: tz,
        notes: sig.institutionalAnalysis?.summary,
      });
    },
  ...
  const handleLogin = () => {
    ...
    setTimeout(() => {
      ...
      if (foundUser || token === masterToken) {
        const user = foundUser || { id: 'admin', name: 'Master Admin', accessId: token, role: 'admin', plan: 'Enterprise', status: 'active', createdAt: new Date().toISOString(), settings: settings };
  ...
  if (!currentUser) return (
    <div className="fixed inset-0 flex items-center justify-center p-6 bg-[#020204]">
      ...
      <div className="w-full max-w-[360px] flex flex-col items-center relative z-10 animate-view-entry">
        <Logo className="w-32 h-32 mb-14" animate={true} showText={true} />
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden text-white relative">
      <FuturisticBg theme={theme} />
      {safeLiftAlert && <SafeLiftAlert type={safeLiftAlert} onClose={() => setSafeLiftAlert(null)} t={t} />}
      ...
      <main className="flex-1 overflow-y-auto pb-48 custom-scrollbar relative z-10">
        {currentView === 'dashboard' && (
          <PremiumTerminal
            ...
            onSaveSignal={
              currentSignal ? () => saveSignalToHistory(currentSignal) : undefined
            }
          />
        )}

        {currentView === 'history' && (
          <HistoryPanel history={history} onDelete={(id) => setHistory(h => h.filter(x => x.id !== id))} />
        )}
              ...
              <div className="flex flex-col items-center gap-8 relative z-10">
                ...
                <div className="text-center">
                  ...
                  <div className="flex items-center justify-center gap-4 mt-3">
                    <span className="text-[10px] font-black text-blue-400 bg-blue-500/10 px-5 py-2 rounded-2xl border border-blue-500/20 uppercase tracking-widest italic">{currentUser.plan} PROTOCOL</span>
                  </div>
                </div>
              </div>
      ...
      <SaveOperationModal
        ...
        defaultAsset={currentSignal?.asset ?? selectedAsset.symbol}
        defaultType={currentSignal?.type}
        defaultTimeframe={selectedTimeframe}
        defaultConfidence={winProbability ?? currentSignal?.score}
        defaultEntry={currentSignal?.entry}
        defaultStop={currentSignal?.sl}
        defaultTake={currentSignal?.tp1}
        defaultInvalidation={currentSignal?.invalidation}
        defaultTimingMode={currentSignal?.timingMode ?? signalTimingMode}
        userId={currentUser?.id}
        userName={currentUser?.name}
      />
