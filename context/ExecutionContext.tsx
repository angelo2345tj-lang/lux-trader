...
import { validateEnv, getEnvWarnings } from '../services/security/envValidation';
...
export function ExecutionProvider({ children }: { children: React.ReactNode }) {
  ...
  const envWarnings = useMemo(() => getEnvWarnings(validateEnv()), []);

  useEffect(() => {
    engine.init();
    return auditLog.subscribe(setAuditEntries);
  }, [engine]);
